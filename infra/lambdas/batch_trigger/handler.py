...
Fires on s3:ObjectCreated for `<bucket>/<incoming_prefix>/<batch_id>/manifest.json`.
Validates the manifest, builds BatchRunInput from env vars (set by terraform's
aws_lambda_function environment block), and starts a BatchRunWorkflow keyed
by `batch_id` with REJECT_DUPLICATE — re-uploads of the same manifest are
idempotent no-ops.

The workflow itself is referenced by string name (`"BatchRunWorkflow"`) so
this Lambda doesn't import any workflow / activity code — only leaf modules
needed for input validation + workflow ID construction. Cuts the deploy
bundle to ~the temporalio + pydantic + pyyaml install plus a handful of
.py files.

Failure modes:
  - Manifest schema invalid           → log + raise (S3 retries; DLQ catches).
  - batch_id mismatch (key vs body)   → log + raise (same).
  - Temporal unreachable              → log + raise (same).
  - Workflow already running          → idempotent success, no raise.

Bundled assets (built by infra/lambdas/batch_trigger/build.sh):
  - prod/batch/{models,planner}.py + workflows/models.py
  - prod/shared_infra/task_queues.py
  - shared/temporal_client.py
  - pipeline_config.yaml, prod_config.yaml  (operator's prod-config snapshot)
"""
...
def _shallow_deep_merge(base: dict, overrides: dict) -> dict:
    """Mirror of BatchRunWorkflow._merge_config — one-level deep over dict
    sections. Duplicated to keep this Lambda free of workflow imports."""
    if not overrides:
        return base
    merged = {**base}
...
def _load_pipeline_config() -> dict:
    """Load + merge bundled configs. yaml.safe_load only — NOT
    load_pipeline_config, because its relative-path anchoring would resolve
    against /var/task (the Lambda's read-only mount), not the worker's box.
    The bundled prod_config.yaml's `pipeline_overrides` block supplies the
    absolute paths the worker actually uses."""
...
# Loaded once on cold start — config is immutable for the Lambda's lifetime.
...
async def _handle(event: dict) -> dict:
    ...
    for record in event.get("Records", []):
        ...
        # S3 notification filters already do prefix + suffix; these are
        # defense-in-depth and rule out a misconfigured notification firing
        # on PDF puts.
        ...
        if manifest.batch_id != batch_id_from_key:
            # Refuse rather than guess which one is right — the operator's
            # uploader is the bug, fix at the source.
            raise ValueError(
