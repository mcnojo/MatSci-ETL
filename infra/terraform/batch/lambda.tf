...
# REJECT_DUPLICATE. The workflow itself owns lifecycle (Phase C) — this
# Lambda is just the trigger.
#
# Bundle: infra/lambdas/batch_trigger/build.sh prepares build/bundle/; the
# archive_file data source below zips it at plan time. Re-run build.sh and
# `terraform apply` whenever the handler or bundled configs change.
...
data "aws_iam_policy_document" "batch_trigger_lambda" {
  statement {
    ...
    resources = [
      "arn:${data.aws_partition.current.partition}:s3:::${var.artifact_bucket}/${var.incoming_prefix}*",
    ]
  }
...
resource "aws_lambda_function" "batch_trigger" {
  ...
  environment {
    variables = {
      ...
      REPORT_ROOT                   = var.batch_report_root
