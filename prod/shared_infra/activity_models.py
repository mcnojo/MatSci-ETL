"""Per-call activity I/O models — shared between live and batch.

These models describe individual LLM/OCR call activities (the granular units
that fan out under the umbrella of a workflow). They are mode-agnostic: both
ProcessPdfWorkflow (live) and BatchRunWorkflow (batch) drive the same
activities and therefore need the same I/O contracts.

Workflow-level models (e.g. ProcessPdfWorkflowInput) stay alongside the
workflow that owns them, under prod/live/ or prod/batch/.
"""
