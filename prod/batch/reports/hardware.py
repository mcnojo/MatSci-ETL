...
The CloudWatch Agent on each worker (configured in user_data.sh.tpl) publishes
into the `OCR/Batch/Worker` namespace at 10s resolution. Instances are enumerated
by listing one always-present metric (`procstat_cpu_usage`).
...
from .models import StatSummary, WorkerInstanceStats

_NAMESPACE = "OCR/Batch/Worker"
_WORKER_PROCESS_PATTERN = "prod.live.worker"
...
def fetch_hardware_stats(
    ...
    *,
    ...
    query_start = start - timedelta(seconds=60)
    query_end = end + timedelta(seconds=120)

    instance_ids = _enumerate_instances(cw, namespace)
    if not instance_ids:
        return [], period
    ...
    for inst in instance_ids:
        ...
        queries = _build_queries(inst, interfaces, namespace, period)
        results = _run_queries(cw, queries, query_start, query_end)
        stats.append(_aggregate_instance(inst, results, interfaces))
    return stats, period
...
def _enumerate_instances(cw, namespace: str) -> list[str]:
    instance_ids: set[str] = set()
    paginator = cw.get_paginator("list_metrics")
    for page in paginator.paginate(Namespace=namespace, MetricName="procstat_cpu_usage"):
...
def _build_queries(
    ...
    procstat_dims = [
        {"Name": "pattern", "Value": _WORKER_PROCESS_PATTERN},
        {"Name": "pid_finder", "Value": "native"},
        *base_inst,
    ]
    queries = [
        _query("cpu_idle", namespace, "cpu_usage_idle",
...
def _aggregate_instance(
    ...
    # cpu_usage_idle → cpu_active for readability.
    cpu_idle = results.get("cpu_idle", _MetricResult([], []))
    cpu_active = _summarize([100.0 - v for v in cpu_idle.values])

    mem_used = _summarize(results.get("mem_used_pct", _MetricResult([], [])).values)
    worker_cpu = _summarize(results.get("worker_cpu", _MetricResult([], [])).values)
    worker_rss = _summarize(results.get("worker_rss", _MetricResult([], [])).values)
...
def _aggregate_net(
    ...
    if not by_ts:
        return None
    return _summarize(list(by_ts.values()))
...
async def fetch_hardware_stats_async(
    ...
    return await asyncio.to_thread(fetch_hardware_stats, region, start, end)
