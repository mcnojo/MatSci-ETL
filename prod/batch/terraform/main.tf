...
# shared/temporal owns the artifact bucket + cpu-pipeline-01 + its SG. We read:
# the bucket name for S3 notification + IAM grants; the SG ID to add a worker
# ingress rule on :7233; the private IP to wire workers to Temporal.
